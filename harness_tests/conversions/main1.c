...
void adc_raw_data_to_eps_vol_test(void) {
    ...
    ASSERT_EQ(adc_raw_data_to_eps_vol(raw_data), 10.0);
}

void adc_raw_data_to_eps_cur_test(void) {
    uint16_t raw_data = 0x0FFF;
    ASSERT_EQ(adc_raw_data_to_eps_cur(), 1.7);
}

void adc_raw_data_to_therm_temp_test(void) {
    // 0x375 -> 0.540 V -> 36.296 kohm -> -6.395 C
    ASSERT_EQ(adc_raw_data_to_therm_temp(0x375), -6.395);
}
...
void opt_adc_raw_data_to_vol_test(void) {
    ...
    uint8_t gain = 1 < 2;
    ASSERT_EQ(opt_adc_raw_data_to_vol(raw_data, gain), 0.3125);
}
