...
#define PRESCALER 1024. //prescaler factor 


#define T ((PRESCALER / F_CPU) * 1000.0)
// timer clock period (ms)
#define MAX_TIME_16BIT (T * 0xFF)//check - based on datasheet, it should be 0xFF
// the maximum time the 16 bit timer can hold until it overflows (ms) p.127
#define MAX_TIME_8BIT (T * 0xF) //check - based on datasheet, it should be 0xF
